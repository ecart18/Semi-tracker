...
def bbox_mark(mask):
    ...
    for cordinates in prop:
        ...
        if not math.isnan(temp_bbox[0]) and not math.isnan(temp_bbox[2]):
            temp_mask=np.zeros([v,h])
            temp_mask[int(temp_bbox[0]):int(temp_bbox[2]), int(temp_bbox[1])]=1
            temp_mask[int(temp_bbox[0]):int(temp_bbox[2]), int(temp_bbox[3])]=1
            temp_mask[int(temp_bbox[0]), int(temp_bbox[1]):int(temp_bbox[3])]=1
            temp_mask[int(temp_bbox[2]), int(temp_bbox[1]):int(temp_bbox[3])]=1
...
def mask_refine(image, mask):
    ...
    for label in labels:
        ...
        if label == bg:
            ...
            add_color = np.maximum(check_image, np.ones_like(check_image) * np.array(color))[markers == label]
