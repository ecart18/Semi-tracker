...
class Writer(Visualization):
    ...
    def __call__(self, frames, output_csv=True, output_html=True, output_img=True):
        ...
        instance_info_label_id = self._generate_statis(instance_info_label_id, output_csv, output_html)
        if output_html:
    ...
    def _generate_statis(self, instance_info_label_id, output_csv, output_html):
        ...
        for label_id in list(instance_info_label_id.keys()):
            ...
            row_names.extend(['instance_%s_area' % label_id, 'instance_%s_intensity' % label_id])
            ...
            mean_intensity = [area * instensity for area, instensity in zip(filter_areas, filter_intensities)]
            mean_intensity = float(np.sum(mean_intensity)) / float(np.sum(filter_areas))
            ...
            if output_html:
                ...
                plot_double_line(filter_frame_index, filter_areas, filter_intensities, label_id, save_name)
            # save as cvs file
            if output_csv:
                results_pd = pd.DataFrame(results, index=row_names, columns=col_names, dtype=np.float)
    ...
    def _generate_html(self, instance_info_label_id):
        ...
        with open(osp.join(self._write_folder, "results.html"),'w+') as html_file:   
            html_content = template.render(frame_num=self.frame_num, img_size=img_size, infos=infos)
