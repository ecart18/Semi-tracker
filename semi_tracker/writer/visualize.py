...
def plot_double_line(frame_index, areas, intensity, instance_id, save_name):
    ...
    line_intensity = ax1.plot(frame_index, intensity, 'b--o', 
                                ...
                                label='Intensity')
    ax1.set_xlabel('Frame Index', fontdict={'family' : PlotFormat.fonttype, 'size': PlotFormat.label_size})
    ax1.set_ylabel('Mean Intensity (A.U)', fontdict={'family':PlotFormat.fonttype, 'size':PlotFormat.label_size})
...
class Visualization(object):
    ...
    def _add_trajectory(self, visual_img, label_id, instance_info_label_id, frame_id, **kwargs):
        ...
        instances = [ins_tracklet[f_idx] for f_idx in range(frame_id) if f_idx in ins_tracklet.keys()]
        if len(instances) > 0:
            start = 0
    ...
    def _add_attr_per_frame(self, instance_info_label_id, frame, frame_index):
        ...
        for _, ins in frame.instances.items(): 
            visual_img = self._add_attr_per_ins(ins, instance_info_label_id, visual_img)
        return visual_img

    def _to_mp4(self, visual_img, visual_folder):
        if not self.video_hander:
            video_file_name = osp.join(visual_folder, 'visualization_video.avi')
            height, width = visual_img.shape[0:2]
    ...
    def _visual(self, instance_info_label_id, frames):
        ...
        for frame_index, frame in frames.items():
            visual_img = self._add_attr_per_frame(instance_info_label_id, frame, frame_index)
